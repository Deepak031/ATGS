...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"),
 "SELECT * FROM teachers WHERE faculty_number = '$whose'");
$row = mysqli_fetch_assoc($query);
$whose_name = $row['name'];
$query = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"), 
"SELECT * FROM teachers WHERE faculty_number = '$sub'");
$row = mysqli_fetch_assoc($query);
$sub_name = $row['name'];
...
if ($row[$period] == "-<br>-" || $row[$period] == "-<br>" || $row[$period] == "-") {
	...
	alert('Selected substitute teacher is not available!');
        window.location.href = 'generatetimetable.php?display='$whose;</script>";
}

$query = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"),
 "SELECT * FROM $whose WHERE day = '$day'");
$row = mysqli_fetch_assoc($query);
$pieces = explode("<br>", $row[$period]);
$string = "Hello " . $sub_name . ",
 You have to take class " . $pieces[0] . " of " . $whose_name . " in " . $pieces[1] . 
 "\n\n-Sent from ATGS GPC BGT";
$_SESSION['s'] = $string;
echo 'Sending SMS...';

if (isset($_POST['pwd'])) {
    echo "<script type='text/javascript'>alert('Message Sent!');
        window.location.href = 'generatetimetable.php?display=" . $whose . "';
		</script>";
?>
<div class="content">
