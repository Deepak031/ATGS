...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"), "SELECT * FROM subjects ");
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"),
...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"), "SELECT * FROM teachers ");
...
/** Genrating timetable for theory courses, with maximum class for each subject equal to 6 */
for ($I = 0; $I < $subjects_count * 6; $I++) {
    $i = $I % $subjects_count;
    $sem = $subjects[$i]->semester;
    $year = ($sem + 1) / 2;
	echo $year;
    $classroom_query = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"),
...
period6='" . $subjectslots[$i][$j][5] . "<br>" . $aliasslots[$i][$j][5][0] . ", " . $aliasslots[$i][$j][5][1] . ", " . $aliasslots[$i][$j][5][2] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "atgs"), $query);
    }
